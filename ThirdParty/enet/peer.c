...
*/
void
enet_peer_disconnect_now (ENetPeer * peer, const SoENetUserData* pUserData)
{
    ENetProtocol command;
        ...
        peer -> state != ENET_PEER_STATE_DISCONNECTING)
    {
        ...
        command.header.channelID = 0xFF;
		SoENetUserData_Host2Net(pUserData, &(command.disconnect.DisconnectUserData));
...
*/
void
enet_peer_disconnect (ENetPeer * peer, const SoENetUserData* pUserData)
{
    ENetProtocol command;
	...
	//Add by oil
	//把UserData存储在peer对象内，当收到ENET_EVENT_TYPE_DISCONNECT事件时
	//就能收到此时的断开连接信息。
	SoENetUserData_Copy(&(peer->eventData), pUserData);
}
...
*/
void
enet_peer_disconnect_later (ENetPeer * peer, const SoENetUserData* pUserData)
{   
    if ((peer -> state == ENET_PEER_STATE_CONNECTED || peer -> state == ENET_PEER_STATE_DISCONNECT_LATER) && 
           ...
           enet_list_empty (& peer -> sentReliableCommands)))
    {
      ...
      enet_peer_disconnect (peer, pUserData);
}
