...
enet_protocol_dispatch_incoming_commands (ENetHost * host, ENetEvent * event)
{
    while (! enet_list_empty (& host -> dispatchQueue))
    {
       ...
       case ENET_PEER_STATE_CONNECTION_SUCCEEDED:
           ...
           event -> peer = peer;
		   SoENetUserData_Copy(&(event->UserData), &(peer->eventData));
		   //Add by oil
		   //把peer->eventData数据清零。
		   SoENetUserData_Clear(&(peer->eventData));
...
//Add by oil
//--bTimeout 记录是否为连接超时导致的断开。
static void
enet_protocol_notify_disconnect (ENetHost * host, ENetPeer * peer, ENetEvent * event, int bTimeout)
{
    ...
    if (event != NULL)
    {
		...
		if (bTimeout)
		{
			event->UserData.UserDataType = UserData_DisconnectTimeout;
			event->UserData.DisconnectReason = DisconnectReason_Timeout;
		}
		else
		{
			SoENetUserData_Copy(&(event->UserData), &(peer->eventData));
...
enet_protocol_handle_connect (ENetHost * host, ENetProtocolHeader * header, ENetProtocol * command)
{
	...
	SoENetUserData_Net2Host(&(command->connect.ConnectUserData), &(currentPeer->eventData));
...
enet_protocol_handle_disconnect (ENetHost * host, ENetPeer * peer, const ENetProtocol * command)
{
	...
	{
	  SoENetUserData_Net2Host(&(command->disconnect.DisconnectUserData), &(peer->eventData));
	}
